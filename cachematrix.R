...
## already been calculated. If so, it gets the mean
## from the cache and skips the computation. 
## Otherwise, it calculates the mean of the data
## and sets the value of the mean in the cache via
...
cachemean <- function(x, ...) {
        ...
        if(!is.null(m)) {                       # if the mean was cached
                ...
                return(m)                       # exit program without running subsequent code
        }
        data <- x$get()                         # otherwise, put data in "data" 
